    ...
    <>
      <div className="mx-auto flex w-full max-w-[1800px] flex-col gap-6 p-6 lg:p-8">
        <section className="grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_420px]">
          <div className={cn('rounded-[30px] border p-6 lg:p-7', mode === 'simple' ? 'border-slate-200 bg-white shadow-[0_8px_22px_rgba(15,23,42,0.045)]' : 'border-white/[0.08] bg-[rgba(13,22,34,0.86)] shadow-[0_20px_64px_rgba(0,0,0,0.22)]')}>
            ...
            <div className="mt-6 flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
              <div className="max-w-3xl">
                <h2 className={cn('text-3xl font-semibold lg:text-[2.2rem]', mode === 'simple' ? 'text-slate-900' : 'text-white')}>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <Link
                  href="/workers"
                  className={cn('inline-flex items-center gap-2 rounded-[18px] border px-4 py-3 text-sm font-medium transition', mode === 'simple' ? 'border-slate-200 bg-white text-slate-800 hover:border-slate-300' : 'border-white/[0.08] bg-white/[0.04] text-slate-100 hover:border-white/[0.15] hover:bg-white/[0.06]')}
                >
                  <Bot className={cn('h-4 w-4', mode === 'simple' ? 'text-cyan-700' : 'text-cyan-100')} />
                  Ver Workers
                </Link>
              </div>
            </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
            {[
              ...
              <div key={item.label} className={cn('rounded-[24px] border px-4 py-4', mode === 'simple' ? 'border-slate-200 bg-white shadow-[0_8px_22px_rgba(15,23,42,0.045)]' : 'border-white/[0.08] bg-[rgba(13,22,34,0.86)] shadow-[0_20px_64px_rgba(0,0,0,0.18)]')}>
                <div className="flex items-center gap-3">
                  <div className={cn('flex h-10 w-10 items-center justify-center rounded-2xl border', mode === 'simple' ? 'border-slate-200 bg-slate-50 text-cyan-700' : 'border-white/[0.08] bg-white/[0.04] text-cyan-100')}>
                    <item.icon className="h-4 w-4" />
                  </div>
                  <div>
                    <p className="text-[11px] uppercase tracking-[0.24em] text-slate-500">{item.label}</p>
                    <p className={cn('mt-1 text-2xl font-semibold', mode === 'simple' ? 'text-slate-900' : 'text-white')}>{item.value}</p>
                  </div>
                </div>
              </div>
            ))}

            <div className={cn('rounded-[24px] border px-4 py-4', mode === 'simple' ? 'border-slate-200 bg-white shadow-[0_8px_22px_rgba(15,23,42,0.045)]' : 'border-white/[0.08] bg-[rgba(13,22,34,0.86)] shadow-[0_20px_64px_rgba(0,0,0,0.18)]')}>
              <p className="text-[11px] uppercase tracking-[0.24em] text-slate-500">Leitura atual</p>
              <p className={cn('mt-3 text-sm font-medium', mode === 'simple' ? 'text-slate-900' : 'text-white')}>
                {selectedCard ? selectedCard.title : 'Selecione um card'}
              </p>
              <p className={cn('mt-2 text-sm leading-6', mode === 'simple' ? 'text-slate-500' : 'text-slate-400')}>
        ...
        <section className="grid gap-5 xl:grid-cols-[1.05fr_0.95fr]">
          <div className={cn('rounded-[30px] border p-5', mode === 'simple' ? 'border-slate-200 bg-white shadow-[0_10px_28px_rgba(15,23,42,0.05)]' : 'border-white/[0.08] bg-[rgba(13,22,34,0.86)] shadow-[0_20px_72px_rgba(0,0,0,0.18)]')}>
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-[11px] uppercase tracking-[0.28em] text-slate-500">Workers</p>
                <h3 className={cn('mt-2 text-xl font-semibold', mode === 'simple' ? 'text-slate-900' : 'text-white')}>Pulso dos agentes</h3>
              </div>
              <Link href="/workers" className="text-sm font-medium text-cyan-200 transition hover:text-cyan-100">
                Abrir tela completa
              </Link>
            </div>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-4">
              {[
                ...
                <div key={metric.label} className={cn('rounded-[22px] border px-4 py-3', mode === 'simple' ? 'border-slate-200 bg-[#f8f9fb]' : 'border-white/[0.08] bg-white/[0.03]')}>
                  <p className="text-[10px] uppercase tracking-[0.2em] text-slate-500">{metric.label}</p>
                  <p className={cn('mt-2 text-xl font-semibold', mode === 'simple' ? 'text-slate-900' : 'text-white')}>{metric.value}</p>
                </div>
              ))}
            </div>
            ...
            <div className="mt-5 space-y-3">
              {workers.slice(0, 3).map((worker) => (
                <div key={worker.id} className={cn('rounded-[24px] border px-4 py-4', mode === 'simple' ? 'border-slate-200 bg-[#f8f9fb]' : 'border-white/[0.08] bg-white/[0.03]')}>
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className={cn('text-sm font-semibold', mode === 'simple' ? 'text-slate-900' : 'text-white')}>{worker.name}</p>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2 text-xs">
                    <span className={cn('rounded-full px-3 py-1', mode === 'simple' ? 'bg-white text-slate-600' : 'bg-white/[0.06] text-slate-300')}>Autônomos {worker.metrics.autopilot}</span>
                    <span className={cn('rounded-full px-3 py-1', mode === 'simple' ? 'bg-white text-slate-600' : 'bg-white/[0.06] text-slate-300')}>Takeover {worker.metrics.takeover}</span>
                    <span className={cn('rounded-full px-3 py-1', mode === 'simple' ? 'bg-white text-slate-600' : 'bg-white/[0.06] text-slate-300')}>Delay {worker.metrics.waitingDelay}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className={cn('rounded-[30px] border p-5', mode === 'simple' ? 'border-slate-200 bg-white shadow-[0_10px_28px_rgba(15,23,42,0.05)]' : 'border-white/[0.08] bg-[rgba(13,22,34,0.86)] shadow-[0_20px_72px_rgba(0,0,0,0.18)]')}>
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <h3 className={cn('mt-2 text-xl font-semibold', mode === 'simple' ? 'text-slate-900' : 'text-white')}>Threads que exigem leitura</h3>
              </div>
              <Link href="/inbox" className="text-sm font-medium text-cyan-200 transition hover:text-cyan-100">
                Abrir inbox
              </Link>
            </div>
            ...
            <div className="mt-5 space-y-3">
              {threads.slice(0, 4).map((thread) => (
                <div key={thread.contact.id} className={cn('rounded-[24px] border px-4 py-4', mode === 'simple' ? 'border-slate-200 bg-[#f8f9fb]' : 'border-white/[0.08] bg-white/[0.03]')}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className={cn('truncate text-sm font-semibold', mode === 'simple' ? 'text-slate-900' : 'text-white')}>{thread.contact.name}</p>
                  ...
                  <p className={cn('mt-3 line-clamp-2 text-sm leading-6', mode === 'simple' ? 'text-slate-600' : 'text-slate-300')}>
        ...
        <section className={cn('rounded-[30px] border p-5', mode === 'simple' ? 'border-slate-200 bg-white shadow-[0_10px_28px_rgba(15,23,42,0.05)]' : 'border-white/[0.08] bg-[rgba(13,22,34,0.86)] shadow-[0_20px_72px_rgba(0,0,0,0.18)]')}>
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-[11px] uppercase tracking-[0.28em] text-slate-500">Pipeline principal</p>
              <h3 className={cn('mt-2 text-xl font-semibold', mode === 'simple' ? 'text-slate-900' : 'text-white')}>
        ...
        {isLoading ? (
          <div className={cn('flex min-h-[46rem] items-center justify-center rounded-[30px] border', mode === 'simple' ? 'border-slate-200 bg-white text-slate-500' : 'border-white/[0.08] bg-[rgba(13,22,34,0.86)] text-slate-400')}>
            Carregando board principal...
          </div>
        ) : error ? (
          <div className="flex min-h-[36rem] items-center justify-center rounded-[30px] border border-rose-400/20 bg-rose-500/10 px-6 text-center text-rose-100">
            {error}
          </div>
        ) : selectedPipelineId ? (
          <div className={cn('rounded-[30px]', mode === 'simple' ? 'bg-transparent' : 'border border-white/[0.08] bg-transparent')}>
            <KanbanBoard
          ...
          <div className={cn('flex min-h-[36rem] items-center justify-center rounded-[30px] border px-6 text-center', mode === 'simple' ? 'border-slate-200 bg-white text-slate-500' : 'border-white/[0.08] bg-[rgba(13,22,34,0.86)] text-slate-400')}>

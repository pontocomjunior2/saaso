...
export type ProspectQueueTaskType = 'research' | 'enrichment';

interface ProspectQueuePayload {
  taskType: ProspectQueueTaskType;
  taskId: string;
}

export interface ProspectQueueRuntimeSnapshot {
...
@Injectable()
export class ProspectQueueService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(ProspectQueueService.name);
  private readonly queueName = 'prospect_execute';
  private readonly queueDriverPreference =
    (process.env.PROSPECT_RUNTIME_DRIVER?.trim().toLowerCase() ?? 'bullmq') as
      | 'bullmq'
      | 'poller';
  ...
  private readonly workerConcurrency = Math.max(
    ...
    this.parsePositiveInteger(
      process.env.PROSPECT_RUNTIME_WORKER_CONCURRENCY,
      4,
    ),
  );
  private queue: Queue<ProspectQueuePayload> | null = null;
  private worker: Worker<ProspectQueuePayload> | null = null;
  ...
  constructor(
    @Inject(forwardRef(() => ProspectService))
  ...
  public async onModuleInit(): Promise<void> {
    ...
    if (this.queueDriverPreference === 'poller') {
      this.logger.log(
        'Prospect runtime configurado para usar apenas o poller interno.',
      );
      return;
    }
    ...
    try {
      ...
      this.queue = new Queue<ProspectQueuePayload>(this.queueName, {
      ...
      this.worker = new Worker<ProspectQueuePayload>(
        this.queueName,
        async (job) => {
          this.lastProcessedAt = new Date();
          const result = await this.prospectService.processQueuedTaskById(
            job.data.taskType,
            job.data.taskId,
          );
      ...
      this.worker.on('error', (error) => {
        ...
        this.logger.error(
          'Worker do runtime de Prospect falhou. O poller interno permanece como fallback.',
          error.stack,
        );
      });
      ...
      this.logger.log(
        `BullMQ operacional para Prospect em redis://${this.redisHost}:${this.redisPort}/${this.redisDb}.`,
      );
    } catch (error) {
      const message =
      ...
      this.queueOperational = false;
      this.logger.warn(
        `BullMQ indisponivel para Prospect (${message}). O runtime segue com o poller interno.`,
      );
  ...
  public getRuntimeSnapshot(): ProspectQueueRuntimeSnapshot {
    return {
  ...
  public async enqueueProspectTask(
    taskType: ProspectQueueTaskType,
    taskId: string,
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : 'Erro desconhecido ao enfileirar job de prospect.';
      this.lastQueueError = message;
      this.logger.error(
        `Falha ao enfileirar tarefa ${taskType}:${taskId} no BullMQ. O poller interno segue como fallback.`,
      );
      return false;
    }
  }

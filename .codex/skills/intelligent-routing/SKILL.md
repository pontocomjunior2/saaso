...
> Codex adaptation:
> - Source: `.agent/skills/intelligent-routing/SKILL.md`
...
| ------------------- | ------------------------------------------ | ------------------------------------------- | ------------ |
| **Authentication**  | "login", "auth", "signup", "password"      | `security-auditor` + `backend-specialist`   | [ok] YES       |
| **UI Component**    | "button", "card", "layout", "style"        | `frontend-specialist`                       | [ok] YES       |
| **Mobile UI**       | "screen", "navigation", "touch", "gesture" | `mobile-developer`                          | [ok] YES       |
| **API Endpoint**    | "endpoint", "route", "API", "POST", "GET"  | `backend-specialist`                        | [ok] YES       |
| **Database**        | "schema", "migration", "query", "table"    | `database-architect` + `backend-specialist` | [ok] YES       |
| **Bug Fix**         | "error", "bug", "not working", "broken"    | `debugger`                                  | [ok] YES       |
| **Test**            | "test", "coverage", "unit", "e2e"          | `test-engineer`                             | [ok] YES       |
| **Deployment**      | "deploy", "production", "CI/CD", "docker"  | `devops-engineer`                           | [ok] YES       |
| **Security Review** | "security", "vulnerability", "exploit"     | `security-auditor` + `penetration-tester`   | [ok] YES       |
| **Performance**     | "slow", "optimize", "performance", "speed" | `performance-optimizer`                     | [ok] YES       |
...
- [ok] User sees which expertise is being applied
- [ok] Transparent decision-making
- [ok] Still automatic (no /commands needed)
...
- [ok] Analyze silently
- [ok] Inform which agent is being applied
- [x] Avoid verbose meta-commentary
...
```markdown
[idea] **Tip**: I am configured with automatic specialist agent selection.
...
[ok] Zero-command operation (no need for `/orchestrate`)  
[ok] Automatic specialist selection based on request analysis  
[ok] Transparent communication of which expertise is being applied  
[ok] Seamless integration with existing workflows  
[ok] Override capability for explicit agent mentions  
[ok] Fallback to orchestrator for complex tasks
